    ...
    <div class="card mx-auto shadow text-center mb-3 h-100 d-flex flex-column justify-content-evenly align-items-center">
        ...
        <div class="card-body d-flex flex-column">
            ...
            <div class="mt-auto my-3 d-flex justify-content-center align-items-center">
                <a href="{{ route('article.show', compact('article')) }}" class="btn mx-2 btn-outline-primary">{{ __('ui.detail') }}</a>
                <a href="{{ route('byCategory', ['category' => $article->category]) }}" class="btn mx-2 btn-outline-secondary">{{ __("ui." . $article->category->name) }}</a>
                @if (Auth::id() == $article->user->id)
                    <a href="{{ route('edit.article', ['article' => $article]) }}" class="btn mx-2 btn-outline-dark">{{ __('ui.edit') }}</a>
                @endif
            </div>
        </div>
    </div>
